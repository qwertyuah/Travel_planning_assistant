...
class ChatRequest(BaseModel):
    """对话模式请求"""
    message: str = Field(..., description="用户输入的消息", example="帮我规划一下重庆三日游")
    session_id: Optional[str] = Field(default=None, description="会话ID，用于维持上下文")
    # 携带当前对话已收集的上下文信息，方便后端判断信息完备度
    current_context: Optional[Dict[str, Any]] = Field(default=None, description="当前对话已收集的上下文信息")
...
class ItineraryResponse(BaseModel):
    """双模式统一行程响应外壳"""
    mode: str = Field(..., description="触发模式: chat(对话) / form(表单)")
    source: str = Field(..., description="数据源: amap(高德强结构) / baidu(百度扁平化) / error")
    # 高德模式下的强类型数据 (表单模式 或 信息完备的对话模式触发)
    data: Optional[TripPlan] = Field(default=None, description="高德强结构数据")
    # 百度模式下的扁平化字典 (信息不全的对话模式 或 高德降级触发)
    raw_data: Optional[Dict[str, Any]] = Field(default=None, description="百度扁平化字典数据")
    message: str = Field(default="", description="提示信息(如降级原因、信息缺失追问)")
